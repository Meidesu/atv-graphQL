...
import { criarMontadora } from "@/services/montadoras.service";
...
const AddMontadoraPage: React.FC = () => {
  ...
  } = useForm<MontadoraFormData>();

  const onSubmit = async (data: MontadoraFormData) => {
    try {
      const response: Montadora = await criarMontadora({
        ...data,
        ano_fundacao: Number(data.ano_fundacao),
      });

      alert(`Montadora ${response.nome} cadastrada com sucesso!`);
    } catch (error) {
