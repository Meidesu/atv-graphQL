...
export default async function Montadoras() {
  ...
  //   }
  //   const montadoras: Montadora[] = await response.json();
            ...
            {montadoras.map((montadora) => (
              ...
              >
                <h2 className="text-xl font-bold mb-2">
                  {montadora.nome}{" "}
                  <span className="text-gray-300">#{montadora.id}</span>
                </h2>
                <p className="text-gray-200">País: {montadora.pais}</p>
                <p className="text-gray-200">
                  Ano de Fundação: {montadora.ano_fundacao}
                </p>
                {(montadora.modelos && montadora.modelos.length > 0 && (
                  <Link
                    href={`/montadoras/${montadora.id}/modelos`}
                    className="text-blue-500 hover:text-blue-700 mt-4 inline-block"
                  >
                    Ver Modelos
                  </Link>
                )) || (
                  <span className="text-gray-400 mt-4 inline-block">
